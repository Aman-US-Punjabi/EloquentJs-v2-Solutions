 ...
 * Bean counting
  ...
  by writing "string".charAt(N), similar to how you get its
  length with "s".length. The returned value will be a string
  containing only one character (for example, "b"). The first
  character has position zero, which causes the last one to be
...
function countBs(value) {
  ...
  for (var index = 0; index < lowerCaseString.length; index++) {
    if (lowerCaseString.charAt(index) == "b") {
      numberOfBs++;
    }
  }
  return numberOfBs
...
function countChar(value, letter) {
  var numberOfBs = 0
  var lowerCaseString = value.toLowerCase()
  for (var index = 0; index < lowerCaseString.length; index++) {
    if (lowerCaseString.charAt(index) == letter) {
      numberOfBs++;
    }
  }
  return numberOfBs
